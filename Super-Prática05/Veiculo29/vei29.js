...
while(true){
    ...
    switch(escolha){
        case "1":
            const aceleracao = parseInt(prompt('Digite a quantidade de velocidade que deseja aumetar:'));
            veiculo.aumentarVelocidade(aceleracao);
            break
        case "2":
            const desaceleracao = parseInt(prompt('Digite a quantidade de desaceleracao:'));
            veiculo.diminuirVelocidade(desaceleracao);
            break
        case "3":
            const nomePassageiro = prompt(`Digite o nome do paddagiero(a):`);
