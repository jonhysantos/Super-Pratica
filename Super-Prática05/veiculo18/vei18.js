const veiculo = {
    ...
    adicionarPassageiros:function(nomePassageiro){
        if(this.velocidade === 0 && this.passageiros.length < this.limitePassageiros){
            ...
            alert(`Limite de passageiros atingida.`)
        }
    },
}
...
while(true){
    ...
    switch(escolha){
        case "1":
            const aceleracao = parseFloat(prompt('Digite a quantiade de velocidade que deseja aumentar:'))
            veiculo.aumentarVelocidade(aceleracao)
            break
        case "2":
            const desaceleracao = parseFloat(prompt('Digite a quantidade de desacaleração:'))
