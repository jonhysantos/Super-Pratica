...
while(true){
    ...
    switch(escolha){
        case "1":
            let aceleracao = parseFloat(prompt(`Digite a quantidade de aceleração:`))
            veiculo.aumentarVelocidade(aceleracao)
            break
        case "2":
            let desaceleracao = parseFloat(prompt('Digite a quantidade de desaceleração:'))
            veiculo.diminuirVelocidade(desaceleracao)
            break
        case "3":
            let nomePassageiro = prompt('Digite o nome do passageiro(a)')
