...
while(true){
    ...
    switch(escolha){
        case "1":
            const aceleracao = parseFloat(prompt('Digite a quantiade da velocidade:'))
            veiculo.aumentarVelocidade(aceleracao)
            break
        case "2":
            const desaceleracao = parseFloat(prompt('Digite a quantidade da desaceleração:'))
            veiculo.diminuirVelocidade(desaceleracao)
            break
        case "3":
            const nomePassageiro = prompt('Digite o nome do passageiro(a):')
