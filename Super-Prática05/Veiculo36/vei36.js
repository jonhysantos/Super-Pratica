const veiculo = {
    ...
    aumentarVelocidade:function(aceleracao){
        ...
        alert(`A velocidade foi aumentada para ${this.velocidade}`)
    },
    diminuirVelocidade:function(desacelaracao){
        if(this.velocidade - desacelaracao > 0){
            this.velocidade -= desacelaracao
...
while(true){
    ...
    switch(escolha){
        case "1":
            ...
            break
        case "2":
            const desacelaracao = parseInt(prompt('Digite a quantidade de desaceleração:'))
            veiculo.diminuirVelocidade(desacelaracao)
            break
        case "3":
